...
const REPORTING_RULES = {
  "Junior Architect": { manager: ["Senior Architect"] },
  "Architect Intern": { manager: ["Senior Architect"] },
  "Junior Engineer": { manager: ["Senior Engineer", "Site Engineer"] },
  "Site Engineer": { manager: ["Senior Engineer"] },
  "B D Associate": { manager: ["DIRECTOR-BD"] },
  Administration: { manager: ["Accounts & Admin Head"] },
  "Office Assistant": { manager: ["Accounts & Admin Head"] },
  Receptionist: { manager: ["Accounts & Admin Head"] },
  "Accounts & Admin Head": { manager: ["DIRECTOR-FN"] },
  // Senior level — no manager, only director
  "Senior Architect": {
    manager: ["DIRECTOR-PR", "DIRECTOR-HR", "DIRECTOR-BD", "DIRECTOR-FN"],
  },
  "Senior Engineer": {
    manager: ["DIRECTOR-PR", "DIRECTOR-HR", "DIRECTOR-BD", "DIRECTOR-FN"],
  },
...
// Returns valid manager options for a given employee based on rules
const getManagerOptions = (employee, allEmployees) => {
  if (isDirectorLevel(employee)) return [];

  const rule = REPORTING_RULES[employee.designation];
  if (!rule) {
    // Fallback: anyone with "Senior" in designation
    return allEmployees.filter(
      (e) => e.id !== employee.id && e.designation?.includes("Senior"),
    );
  }

  if (rule.manager.length === 0) return [];

  return allEmployees.filter(
    (e) =>
      e.id !== employee.id &&
      rule.manager.some((m) =>
        e.designation?.toLowerCase().includes(m.toLowerCase()),
      ),
  );
};

// All directors for the director column
const getDirectorOptions = (employee, allEmployees) => {
  if (isDirectorLevel(employee)) return [];
  return allEmployees.filter((e) => isDirectorLevel(e) && e.id !== employee.id);
};
...
const LeaveApprovalPanel = () => {
  ...
  const fetchHistory = () => {
    ...
    axios
      .get("/get_leave_history", { params: { level: approvalLevel } })
      .then((res) => {
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        {[
          ...
          >
            ...
            <span
              ...
              style={{
                background: actionTab === i ? "#1565c0" : "#f0f2f8",
                color: actionTab === i ? "#fff" : "#94a3b8",
              }}
            >
              {t.count}
            </span>
        ...
        <div className="ml-auto flex items-center gap-2">
          <Chip
            size="sm"
            variant="soft"
            color={isDirector ? "primary" : "success"}
            sx={{ fontWeight: 700, fontSize: "0.7rem" }}
          >
          ...
          {detailLeave && (
            <Box sx={{ p: 3 }}>
              {[
                ...
                [
                  "Leave Type",
                  <LeaveTypeBadge type={detailLeave.leave_type} />,
                ],
...
// ─── Reports To Cell ──────────────────────────────────────────────────────
const ReportsToCell = ({ employee, allEmployees, reportType }) => {
  const options =
    reportType === "director"
      ? getDirectorOptions(employee, allEmployees)
      : getManagerOptions(employee, allEmployees);
  ...
  // // Add this right after the options derivation inside ReportsToCell:
  // const managerIsDirector =
  //   reportType === "director" && isDirectorLevel(savedValue ? savedValue : {});
  ...
  if (isDirectorLevel(employee)) {
    return (
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[0.78rem] font-semibold text-slate-500 italic">
          Top level
        </span>
      </div>
    );
  ...
  if (options.length === 0) {
    return (
      <span className="text-[0.75rem] text-gray-400 italic">
        No options defined
      </span>
    );
  }
  ...
  return (
    <div className="flex items-center gap-2 min-w-0 w-40">
      <Autocomplete
        ...
        placeholder={
          reportType === "director" ? "Select director…" : "Select manager…"
        }
          ...
          >
            <Box
              sx={{
                ...
                background:
                  reportType === "director"
                    ? "linear-gradient(135deg, #b91c1c, #ef4444)" // red for directors
                    : "linear-gradient(135deg, #1565c0, #42a5f5)", // blue for managers
                display: "flex",
                alignItems: "center",
...
const AdminPanel = () => {
  ...
  // Remove the earlier directors/managers filter lines and replace with:
  const directors = employees.filter((e) => isDirectorLevel(e));
  const managers = employees.filter((e) => !isDirectorLevel(e));

  // Add these two derived values right after the employees state is set,
  // just before the return statement in AdminPanel:
          ...
          <Tabs defaultValue={0}>
            ...
            <TabPanel value={0} sx={{ p: 3 }}>
              ...
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                ...
                {[
                  {
                    from: "Junior Architect / Architect Intern",
                    to: "Senior Architect → Director",
                  },
                  {
                    from: "Junior Engineer / Site Engineer",
                    to: "Senior Engineer → Director",
                  },
                  {
                    from: "Senior Architect / Senior Engineer",
                    to: "Director",
                  },
                ].map((rule) => (
                  <span
                    key={rule.from}
                    className="text-[0.68rem] bg-indigo-50 border border-indigo-100 text-indigo-600 px-2 py-0.5 rounded-lg font-medium"
                  >
                    {rule.from} → {rule.to}
                  </span>
                ))}
